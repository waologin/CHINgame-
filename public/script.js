...
const settingChar1 = document.getElementById('setting-char1');
const settingChar2 = document.getElementById('setting-char2');
...
function startLocalGame(isRematch = false) {
    ...
    targetLength = 4;
    currentSymbols = [settingChar1.value || 'ち', settingChar2.value || 'ん'];
    updateSymbolButtons();
...
createRoomBtn.addEventListener('click', () => {
    const settings = {
        char1: settingChar1.value,
        char2: settingChar2.value,
...
socket.on('gameStart', ({ roomId, players, turn, targetLength: len, timeLimit: time, symbols }) => {
    currentRoomId = roomId;
    targetLength = len || 4;
    timeLimit = time || 0;
    currentSymbols = symbols || ['ち', 'ん'];
    updateSymbolButtons();
...
function updateTurn(turn) {
    ...
    if (isMyTurn) {
        ...
        // 他人の番でも文字選択は見えていても良いが、操作不能にすべき
        // ここでは以前の通りhiddenにするか、操作不能にする
...
function updateSymbolButtons() {
    ...
    btns[0].textContent = currentSymbols[0];
    btns[0].dataset.symbol = currentSymbols[0];
    btns[1].textContent = currentSymbols[1];
    btns[1].dataset.symbol = currentSymbols[1];
    
    // 初期選択
    btns.forEach(b => b.classList.remove('selected'));
    btns[0].classList.add('selected');
    mySymbol = currentSymbols[0];
}
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('symbol-btn')) {
        ...
        mySymbol = e.target.dataset.symbol;
    }
});
...
function checkWinLocal(board) {
    const size = 5;
    const baseTarget = currentSymbols;
    const target = [];
