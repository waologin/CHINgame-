...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', (settings = {}) => {
        ...
        const char1 = settings.char1 || 'ち';
        const char2 = settings.char2 || 'ん';
        rooms.set(roomId, {
            players: [socket.id],
            ...
            startingPlayer: 0,
            targetLength: 4, // 4固定
            timeLimit: parseInt(settings.timeLimit) || 0, // 0なら制限なし
            timer: null
        });
        socket.join(roomId);
    ...
    function startTurnTimer(roomId) {
        const room = rooms.get(roomId);
        if (!room || room.status !== 'playing' || room.timeLimit <= 0) return;
    ...
    socket.on('makeMove', ({ roomId, index, symbol }) => {
        ...
        // 不正な文字
        if (!room.symbols.includes(symbol)) {
        ...
        // 勝敗判定
        if (checkWin(room.board, index, room.targetLength, room.symbols)) {
            if (room.timer) {
                clearInterval(room.timer);
                room.timer = null;
            }
...
function checkWin(board, lastIndex, targetLength, symbols) {
    const size = 5;
    const baseTarget = symbols || ['ち', 'ん'];
    const target = [];
